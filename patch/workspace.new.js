window.__ModuleLoader__.load({
	...
	factory: (require) => {
							...
							return (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [
									(0, react_jsx_runtime.jsx)(ProjectRowItem, {
										...
										t,
										onToggle: () => {
											if (group.workspaceId !== void 0) kanbanSetCurrent(group.workspaceId);
									...
									kanbanMode() && group.workspaceId !== void 0 ? (0, react_jsx_runtime.jsx)(KanbanModules, {
										group,
										t
									}) : [
		...
		*/
		/* ── dsh-kanban patch: project list shows module checkboxes as the
		* second level under each workspace (sessions live in the kanban column
		* only). State contract shared with @deepseek-ai/dsh-client-ui-kanban via
		* localStorage key dsh.kanban.v1 + the window "dsh:kanban-change" event. ── */
		...
		function kanbanMode() {
			try { return window.localStorage.getItem("dsh.kanban.v1.enabled") !== "false"; } catch (e) { return true; }
		}
		function kanbanSetCurrent(workspaceId) {
			const st = kanbanRead();
			if (st.current === workspaceId) return;
			st.current = workspaceId;
			kanbanWrite(st);
		}
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=dsh-kanban-sidebar]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-kanban-sidebar";
			tag.dataset.pluginCss = "dsh-kanban-sidebar";
			tag.textContent = ".dsh-kb-mod{display:flex;align-items:center;gap:6px;padding:4px 8px 4px 24px;font-size:12.5px;cursor:pointer;border-radius:8px;user-select:none;color:var(--dsw-alias-label-primary)}.dsh-kb-mod:hover{background:rgba(255,255,255,.7)}.dsh-kb-mod input{accent-color:var(--dsw-alias-accent,#2f6fed)}.dsh-kb-mod-all{font-weight:700;border-bottom:1px solid var(--dsw-alias-border-l1);border-radius:0;margin:2px 8px 4px;padding-left:10px}.dsh-kb-mod-new{display:flex;gap:6px;padding:6px 8px 4px 24px}.dsh-kb-mod-new input{flex:1;min-width:0;font-size:12px;padding:3px 6px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.dsh-kb-mod-new button{border:1px solid var(--dsw-alias-accent,#2f6fed);background:var(--dsw-alias-accent,#2f6fed);color:#fff;border-radius:6px;font-size:12px;padding:2px 8px;cursor:pointer}.dsh-kb-mod-empty{font-size:11.5px;color:var(--dsw-alias-label-secondary);padding:4px 8px 4px 24px}";
			document.head.appendChild(tag);
		}
		function KanbanModuleCreator({ workspaceId }) {
			const [value, setValue] = react.useState("");
			const commit = () => {
				const name = value.trim();
				if (!name) return;
				const next = kanbanRead();
				const m = (next.modules = next.modules || {});
				const list = (m[workspaceId] = m[workspaceId] || []);
				if (!list.includes(name)) list.push(name);
				next.filter = next.filter || {};
				next.filter[workspaceId] = null;
				kanbanWrite(next);
				setValue("");
			};
			return react_jsx_runtime.jsxs("div", {
				className: "dsh-kb-mod-new",
				children: [
		...
		function KanbanModules({ group, t }) {
			...
			const workspaceId = group.workspaceId;
			const st = kanbanRead();
			const mods = (st.modules || {})[workspaceId] || [];
			const filter = (st.filter || {})[workspaceId];
			const sel = filter === void 0 || filter === null ? mods : filter;
			const rows = [];
			rows.push(react_jsx_runtime.jsx("label", {
				className: "dsh-kb-mod dsh-kb-mod-all",
				children: [
					react_jsx_runtime.jsx("input", {
						type: "checkbox",
						checked: mods.length > 0 && sel.length === mods.length,
						onChange: (e) => {
							const next = kanbanRead();
							const m = (next.modules = next.modules || {})[workspaceId] || [];
							next.filter = next.filter || {};
							next.filter[workspaceId] = e.target.checked ? null : [];
							kanbanWrite(next);
						}
					}),
					react_jsx_runtime.jsx("span", { children: "全选" })
				]
			}, "kanban-all"));
			if (mods.length === 0) {
				rows.push(react_jsx_runtime.jsx("div", {
					className: "dsh-kb-mod-empty",
					children: "暂无模块，可在下方新建"
				}, "kanban-empty"));
			}
			for (const mod of mods) {
				rows.push(react_jsx_runtime.jsx("label", {
					className: "dsh-kb-mod",
					children: [
						react_jsx_runtime.jsx("input", {
							type: "checkbox",
							checked: sel.includes(mod),
							onChange: (e) => {
								const next = kanbanRead();
								const m = (next.modules = next.modules || {})[workspaceId] || [];
								next.filter = next.filter || {};
								let cur = next.filter[workspaceId];
								if (cur === void 0 || cur === null) cur = [...m];
								cur = e.target.checked ? [...cur, mod] : cur.filter((x) => x !== mod);
								next.filter[workspaceId] = cur.length === m.length ? null : cur;
								kanbanWrite(next);
							}
						}),
						react_jsx_runtime.jsx("span", { children: mod })
					]
				}, mod));
			}
			rows.push(react_jsx_runtime.jsx(KanbanModuleCreator, { workspaceId }, "kanban-new"));
			return react_jsx_runtime.jsx(react_jsx_runtime.Fragment, { children: rows });
		}
		/* ── end dsh-kanban module checkboxes ── */
